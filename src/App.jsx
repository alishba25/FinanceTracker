...
export default function App() {
  ...
  // FORCE FULL SCREEN WRAPPER ADDED HERE -> fixed inset-0 w-full h-full
  return (
        ...
        <main className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-10 scroll-smooth">
            <div className="max-w-6xl mx-auto pb-10">
              {currentView === 'dashboard' && <DashboardView stats={stats} formatCurrency={formatCurrency} isDarkMode={isDarkMode} />}
...
function DashboardView({ stats, formatCurrency, isDarkMode }) {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
        <div className="bg-[#003049] p-8 rounded-[2.5rem] text-[#EAE2B7] shadow-xl relative overflow-hidden group border border-white/5 flex flex-col justify-between">
          ...
          <div>
            ...
            <h2 className="text-4xl font-black truncate tracking-tighter">{formatCurrency(stats.balance)}</h2>
          </div>
          <div className="flex gap-4 mt-8 pt-6 border-t border-white/10 shrink-0">
             <div className="flex-1 overflow-hidden"><p className="text-[9px] font-black text-emerald-400 uppercase">Incoming</p><p className="font-bold text-sm truncate">{formatCurrency(stats.income)}</p></div>
             <div className="flex-1 overflow-hidden"><p className="text-[9px] font-black text-rose-400 uppercase">Outgoing</p><p className="font-bold text-sm truncate">{formatCurrency(stats.expense)}</p></div>
          </div>
        </div>
        <Card title="Month Total Income" value={formatCurrency(stats.income)} color={isDarkMode ? COLORS.GOLD : COLORS.DEEP_BLUE} isDarkMode={isDarkMode} />
        <Card title="Month Total Expenses" value={formatCurrency(stats.expense)} color={COLORS.RED} isDarkMode={isDarkMode} />
      </div>
...
function TransactionModal({ onClose, onSave, isDarkMode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      ...
      {/* ADDED max-h-[90vh] AND flex-col TO FIX HEIGHT CUT OFF */}
      <div className="bg-[#FDFBF7] dark:bg-[#003049] rounded-[2.5rem] sm:rounded-[3.5rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white/10 flex flex-col max-h-[90vh]">
        ...
        {/* MODAL FORM (ADDED overflow-y-auto TO ENABLE SCROLLING) */}
